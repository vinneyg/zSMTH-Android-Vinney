...
public interface SMTHWWWService {
    ...
    //+Vinney
    //https://www.newsmth.net/bbsboa.php?group=5&group2=677
    @GET("/bbsboa.php")
    Observable<ResponseBody> getBoardsInGroup(@Query("group") String group,
                                           @Query("group2") String group2);
    //-Vinney

    @GET("/nForum/section/{section}?ajax")
    Observable<ResponseBody> getBoardsBySection(@Path("section") String section);

    // loadChildBoardsRecursivelyFromWWW
